...
hook:
  ...
  fields:
    ...
    - name: pool.count
      reads: 今天选题池 10 条
      writes: derived
      from: 常青 + 案例 RSS + Newsletter RSS + 快报 + 热搜。热搜优先小红书/抖音/B站，微博等仍可进池但排在后面。过主题词；没种主题不过滤。有认的领域（设置里最多 1–3 个，或已种主题）时往这边偏，不硬清空。
      exists_today: true
...
home:
  ...
  - 一句话开工（输入一句 + 开工，不经过热搜）
  - 今日选题池约 10 条：常青 + 案例/Newsletter + 快报 + 热搜。热搜优先小红书/抖音/B站，微博等可以进，不排第一。有认的领域时案例/快报/热搜往这边排。每张交给创作者的是可直接发的标题 + 可直接念的开头，不是「可以去做xx」。点「换一批」必须换卡。
...
故意并掉、不开新格的：AI快报 / 科技 / 开源（进订阅）；低粉高赞（没源，本质是案例切片）；搜索（是带着词去找的工具，不是货架陈列，没稳定源之前不开空壳）；美妆 / 职场 / 地产等垂直领域（那是用户的源和树，不是产品的格子）。
...
- **选题池约 10 条，必须混源。** 常青至少 2 张。热搜优先小红书/抖音/B站，微博等仍可进池、排在后面。过主题词（没种主题则不过滤）。有认的领域时往这边偏。首屏不跑大模型。每张标题必须是可直接发的一句，下面一行必须是可直接念的开头。禁止「可以去做」「适合做态度」。外部源全挂时仍出常青，不准编假题。
- **可执行闸门。** 方向里的标题/Hook、拍写里的开头/口播/正文，必须能照搬。出现「可以考虑」「不妨」「第一段讲」「拍摄方案」就作废重出。做不到成片文件，但字必须能直接用。
...
- **登录门。** 邮箱 + 密码 + 验证码。密码可点小眼睛查看/关闭。每人一份工作区，互相看不见。进门不问行业。没有结账。验证码、限流、蜜罐防机器人。密钥仍只存在本机各自的工作区。
...
- **channels 就是左侧主导航。** `weight: primary` 的「今天」是默认落地页。原料货架六个 tab 是逛法（舞台 / 案例 / 讨论 / 节点 / 订阅 / 对标），不是选题领域。不要再加美妆格、职场格。
