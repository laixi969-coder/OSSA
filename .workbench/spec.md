...
hook:
  ...
  fields:
    ...
    - name: pool.count
      reads: 今天选题池 10 条
      writes: derived
      from: 常青 + 案例 RSS + Newsletter RSS + 快报 + 热搜。热搜优先小红书/抖音/B站，微博等仍可进池但排在后面。过主题词；没种主题不过滤。
      exists_today: true
...
home:
  ...
  - 一句话开工（输入一句 + 开工，不经过热搜）
  - 今日选题池约 10 条：常青 + 案例/Newsletter + 快报 + 热搜。热搜优先小红书/抖音/B站，微博等可以进，不排第一。每条必须有推荐理由。点「换一批」必须换卡。
...
channels:
  - name: 今天
    ...
    tabs:
      - id: platform_hot
        name: 平台热榜
        shows: 三列实时热榜，默认小红书 / 抖音 / B站。微博、知乎可切，不作为进门。条目标题 + 热度。这是货架，不是选题引擎。
        source: 60s API
        empty: 60s 没起来时写「本机 60s 还没开，默认地址 http://127.0.0.1:4399」。某一列读不到就标明，不要用微博顶上。
      - id: ai_brief
        name: AI 快报
...
- **选题池约 10 条，必须混源。** 常青至少 2 张。热搜优先小红书/抖音/B站，微博等仍可进池、排在后面。过主题词（没种主题则不过滤）。货架默认三列主场，可切到微博。每条有推荐理由。外部源全挂时仍出常青，不准编假题。
