...
### 两种变体（不新增高度）
- **news · 转述的原帖**：立场为「当事人回应 / 单方陈述」的剪报，是发酵节点经媒体转述。
  立场行转靛蓝、点出发声者（从标题认：景甜方回应 → 景甜方），标题/摘要里摘得到
  4–60 字带引号原话就进 p 位走引文体；非第一现场时右下盖墨色「转述」章。
  摘不到原话不编。占位与卡高不变。
- **post · 热搜词条**：热榜快照命中的词条不是原帖（无作者）。来源写「微博热搜 /
  知乎热榜 / 抖音热榜 / B站热榜 / 今日头条」，副行写「热搜词条 · 日期/此刻」（榜给得出
  日期就写日期，如知乎；给不出写「此刻」），摘要写位次；不假装是发帖时间。
  数据链路：socialHits 打 hotEntry 标记 → hitsToCards 贯通 → 前端按标记换文案。
  热榜词条在 collectHits 里排在新闻前，36 张截断不先砍它们。
  历史回溯：server 每 30 分钟把五路热榜整榜追加到 data/hotsnap/（UTC 天文件，
  保留 30 天，全实例共享）。socialHits 先查此刻榜，再查存档——已掉榜的词条带
  「首现日期 + 最好位次」上墙（摘要如「热榜第 1 位 · 8-27 上榜」），还在榜的
  用存档补首现日期。快照从部署之日起积累，此前的事件回溯不了。
